...
use normalizer::models::{NormalizedComment, NormalizedIssue, NormalizedRepository, NormalizedUser};
...
impl DataFetcher for StubFetcher {
    ...
    async fn fetch_user(&self, user: &UserRef) -> Result<UserFetch> {
        Ok(UserFetch::Missing(collector::fetcher::MissingUser { id: user.id, login: user.login.clone(), status: None }))
    }
}
...
async fn collector_dedup_repo_registration_across_runs() -> Result<()> {
    ...
    let job = database
        .collection_jobs()
        .create(CollectionJobCreate { owner: "Owner".into(), name: "Example".into(), priority: 0 })
        .await?;

    // First run: register repo with mixed-case full_name
    let fetcher1 = Arc::new(StubFetcher { repo_id: 4242, full_name: "Owner/Example".into() }) as Arc<dyn DataFetcher>;
    let collector1 = Collector::new(test_config(), fetcher1, repos.clone());
    collector1.run_once().await?;

    let count1: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM repositories").fetch_one(database.pool()).await?;
    assert_eq!(count1, 1);

    // Reset job to pending to simulate re-processing
    database
        .collection_jobs()
        .update(CollectionJobUpdate { id: job.id, status: CollectionStatus::Pending, error_message: None })
        .await?;

    // Second run: same repo id, different full_name case
    let fetcher2 = Arc::new(StubFetcher { repo_id: 4242, full_name: "owner/example".into() }) as Arc<dyn DataFetcher>;
    let collector2 = Collector::new(test_config(), fetcher2, repos.clone());
    collector2.run_once().await?;

    let (count2, full_name): (i64, String) = sqlx::query_as(
        "SELECT COUNT(*), MIN(full_name) FROM repositories",
    )
    .fetch_one(database.pool())
    .await?;
    assert_eq!(count2, 1, "still a single repository row after second run");
    assert_eq!(full_name, "owner/example", "full_name updated using latest payload");
